...
int verify(double* x, double* y, void(*funct)(double *x, double *y, double *z)) {
    double *z_v_add = (double*) calloc(ARRAY_SIZE, sizeof(double));
    double *z_oracle = (double*) calloc(ARRAY_SIZE, sizeof(double));
    (*funct)(x, y, z_v_add);
...
char* compute_dotp(SIZE_T arr_size) {
    // Generate input vectors
    char* report_buf = (char*)calloc(1, BUF_SIZE), *pos = report_buf;
