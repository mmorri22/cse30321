...
class first_class{
    ...
    public:
        ...
        // Example Constructor
        first_class() : string_1(), int_1(), vector_1(), char_1(), 
            char_2(), double_1(), float_1(), char_3(), string_2(), vector_2() {}
...
int main(){
    ...
    // Learn the Stored-Program Concept
    COUT << "Size of std::string: " << sizeof(std::string) << ENDL;
    COUT << "Size of std::vector: " << sizeof(std::vector<double>) << ENDL;

    // Print the basic information
    //size_t optimal_size = 2*sizeof(std::string) + 2*sizeof(std::vector<double>) 
    //    + sizeof(double) + sizeof(int) + sizeof(float) + 3*sizeof(char);
